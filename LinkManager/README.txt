...
-- Manually create a link by pasting in the target of the link on the currently
   selected tab.(CTRL + V)
	-- This will prompt to add a name for the link, showing the pasted target
           so the user can verify accurracy prior to insertion.
...
-- Save your configuration to a local XML file, "configuration.xml".
   This file will retain any changes made to the list. This file can be replaced with any configuration.xml file 
   that was made by this program.
...
-- Ctrl + "v" will add a new link to the current tab, prompting for the name. 
	      The data pasted is assumed to be the target.
